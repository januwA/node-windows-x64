...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
...
#else
    typedef CESSTYPE semantic_type;
#endif
    /// Symbol locations.
    ...
    struct token
    {
      ...
      {
        CESEMPTY = -2,
    CESEOF = 0,                    // "end of file"
    CESerror = 256,                // error
    CESUNDEF = 257,                // "invalid token"
    HEX = 258,                     // "hex"
    IDENT = 259,                   // "ident"
    PLUS = 260,                    // "+"
    MINUS = 261,                   // "-"
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, BaseNode*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const BaseNode*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_CESEOF (location_type l)
      {
        return symbol_type (token::CESEOF, std::move (l));
      }
#else
      static
      symbol_type
      make_CESEOF (const location_type& l)
      {
        return symbol_type (token::CESEOF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_CESerror (location_type l)
      {
        return symbol_type (token::CESerror, std::move (l));
      }
#else
      static
      symbol_type
      make_CESerror (const location_type& l)
      {
        return symbol_type (token::CESerror, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_CESUNDEF (location_type l)
      {
        return symbol_type (token::CESUNDEF, std::move (l));
      }
#else
      static
      symbol_type
      make_CESUNDEF (const location_type& l)
      {
        return symbol_type (token::CESUNDEF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
